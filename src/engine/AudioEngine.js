export class AudioEngine {
  ...
  tone(roleId, padId, time) {
    ...
    const t   = time !== null && time !== undefined ? time : this.ctx.currentTime;

    const env = (g, atk, dec, vol) => {
    ...
    if (roleId === "drummer" || roleId === "percussion") {
      ...
      [261, 329, 392].forEach((freq, i) => {
        ...
        g.gain.setValueAtTime(0, t);
        g.gain.linearRampToValueAtTime(0.28 / (i + 1), t + 0.01);
        g.gain.exponentialRampToValueAtTime(0.001, t + dec);
  ...
  schedule() {
    ...
    while (this.nextTime < now + 0.15) {
      ...
      Object.entries(this.patterns).forEach(([padId, pat]) => {
        if (!pat.muted && pat.steps && pat.steps[step] === 1) {
    ...
    this.timerId = setTimeout(() => this.schedule(), 30);
  }
  ...
  tapsToSteps(taps, bpm) {
    if (!taps || !taps.length) return new Array(16).fill(0);
  ...
  detectBPM(taps) {
    if (!taps || taps.length < 3) return this.bpm;
    const intervals = [];
